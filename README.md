...
The server exposes a small P0 toolset:
...
- `create_raw_source` — create a new raw source; existing files are protected by default.
- `append_log` — append a structured `log.md` entry and trim old entries.
...
- `raw/` writes are create-only unless `allow_overwrite=true` is explicit.
